...
def get_oct500(list_ids, data_dir, class_label):
    img_paths = []
    for idd in list_ids:
        file_path = os.path.join(data_dir, "OCT", str(idd))
        for img in os.listdir(file_path):
            if ("6mm" in data_dir and 160 <= int(img[:-4]) <= 240) or \
